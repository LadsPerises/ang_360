...
import { MapPin, ArrowRight, Thermometer } from 'lucide-react';
...
const coordinates: Record<string, { lat: number, lon: number }> = {
  ...
  'Cunene': { lat: -16.0667, lon: 15.0 },
  'Namibe': { lat: -15.1961, lon: 12.1522 },
  'Zaire': { lat: -6.2667, lon: 14.2333 },
  'Uíge': { lat: -7.6083, lon: 15.0611 },
...
export default function AngolaMap() {
  ...
  useEffect(() => {
    if (hoveredProvince && weatherCache[hoveredProvince] === undefined) {
      const fetchWeather = async () => {
        const coords = coordinates[hoveredProvince] || coordinates['default'];
        try {
          ...
          if (data && data.current_weather) {
            setWeatherCache(prev => ({ ...prev, [hoveredProvince]: data.current_weather.temperature }));
          }
        } catch (error) {
  ...
  }, [hoveredProvince, weatherCache]);

  const handleMouseMove = (e: MouseEvent<SVGSVGElement>) => {
    const target = e.target as SVGElement;
    const provinceTag = target.closest('.province');
    if (provinceTag) {
      const province = provinceTag.getAttribute('data-province');
      if (province) {
  ...
  const handleProvinceClick = (e: MouseEvent<SVGPathElement>, province: string) => {
    e.preventDefault();
    
    // Calculate click position relative to map for zoom origin
    ...
    setTimeout(() => {
      navigate(`/explore?province=${encodeURIComponent(province)}`);
    }, 800); // 800ms matches the CSS transition duration
  };

  return (
    <div className="w-full flex justify-center interactive-map-wrapper relative overflow-hidden">
      <style>{`
        #angola-map {
        ...
        #angola-map:not(.zooming) .province:hover {
            fill: #d62626;
            stroke: #d62626;
            filter: drop-shadow(0 0 8px rgba(214, 38, 38, 0.8));
      ...
      {hoveredProvince && !isZooming && (
        ...
        >
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-white font-bold text-lg">{hoveredProvince}</span>
            {weatherCache[hoveredProvince] !== undefined && (
              <span className="ml-auto flex items-center gap-1 text-secondary text-xs font-bold bg-secondary/10 px-2 py-0.5 rounded-md border border-secondary/20 shadow-sm animate-pulse">
                <Thermometer size={12} />
                {weatherCache[hoveredProvince]}°C
              </span>
            )}
          </div>
          <p className="text-white/60 text-xs mb-3 max-w-[200px] leading-relaxed">Clique para explorar o ambiente 360º desta província e descobrir os seus encantos.</p>
          <div className="flex items-center gap-1 text-[10px] uppercase tracking-widest text-secondary font-bold">
            Explorar agora <ArrowRight size={12} />
          </div>
        </div>
      )}
    </div>
